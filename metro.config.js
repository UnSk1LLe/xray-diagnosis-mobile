...
// Ensure Metro cache folders exist before Metro starts writing.
// On Windows, DiskCacheManager doesn't create parent dirs and will throw ENOENT.
...
// Metro's config key differs across versions.
// Prefer `fileMapCacheDirectory` when available, otherwise fall back.
config.fileMapCacheDirectory = cacheDir;
if (!('fileMapCacheDirectory' in config)) {
  // Best-effort fallback; Metro will append its own filename.
  config.fileMapCacheFile = path.join(cacheDir, 'metro-file-map');
}
